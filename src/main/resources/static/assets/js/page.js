        ...
        success: function(json){
            ...
            if(page < 4){
                if(pageInfo.pages>page){
                    for(i = 0; i<5 ;i++){
                        var pageNum = $("<a href='' class='style--pageLi page' onclick='getData(value,window.categoryId)' id='p"+(i+1)+"' value="+(i+1)+">"+(i+1)+"</a>");
                        //拼接
                        if(i == 0){
                            $("#p"+i).append(pageNum);
                        }else{
                    ...
                    for(i=0;i<Math.ceil(pageInfo.list.length / 7);i++){
                         pageNum = $("<a href='' class='style--pageLi page' onclick='getData(value,window.categoryId)'  id='p"+(i+1)+"' value="+(i+1)+" >"+(i+1)+"</a>");
                        //拼接
                        if(i == 0){
                            $("#p"+i).append(pageNum);
                        }else{
                ...
                for(boo ? i = pageInfo.pages - 4 : i = parseInt(page) - 2,y = 0; boo ? i <= pageInfo.pages : i< parseInt(page) + 3 ;i++,y++){
                    var pageNum = $("<a href='' class='style--pageLi page' onclick='getData(value,window.categoryId)' id='p"+(y+1)+"' value="+(i)+">"+(i)+"</a>");
                        //拼接
                        if(y == 0){
                        $("#p"+y).append(pageNum);
                    }else{
